...
install_custom_feed() {
    local feeds_path
    local fullconenat_nft_dir="$BUILD_PATH/package/network/utils/fullconenat-nft"
    local fullconenat_dir="$BUILD_PATH/package/network/utils/fullconenat"
...
collect_missing_directories() {
    ...
    for dir_name in "${required_dirs_ref[@]}"; do
        if [ ! -d "$base_dir/$dir_name" ]; then
            missing_dirs_ref+=("${base_dir#$BUILD_PATH/}/$dir_name")
        fi
    done
}
