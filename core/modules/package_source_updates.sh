...
check_default_settings() {
    local settings_dir="$BUILD_PATH/package/emortal/default-settings"
    if [ -z "$(find "$BUILD_PATH/package" -type d -name "default-settings" -print -quit 2>/dev/null)" ]; then
        echo "在 $BUILD_PATH/package 中未找到 default-settings 目录，正在从 immortalwrt 仓库克隆..."
        local tmp_dir
        tmp_dir=$(mktemp -d)
...
add_ax6600_led() {
    local athena_led_dir="$BUILD_PATH/package/emortal/luci-app-athena-led"
...
add_timecontrol() {
    local timecontrol_dir="$BUILD_PATH/package/luci-app-timecontrol"
...
update_smartdns() {
    ...
    local SMARTDNS_DIR="$BUILD_PATH/feeds/packages/net/smartdns"
    local LUCI_APP_SMARTDNS_REPO="https://github.com/pymumu/luci-app-smartdns.git"
    local LUCI_APP_SMARTDNS_DIR="$BUILD_PATH/feeds/luci/applications/luci-app-smartdns"
...
update_mwan3_fw4() {
    ...
    local mwan3_dir="$BUILD_PATH/feeds/packages/net/mwan3"
    local luci_app_mwan3_dir="$BUILD_PATH/feeds/luci/applications/luci-app-mwan3"
...
update_diskman() {
    local path="$BUILD_PATH/feeds/luci/applications/luci-app-diskman"
    local repo_url="https://github.com/lisaac/luci-app-diskman.git"
    if [ -d "$path" ]; then
        echo "正在更新 diskman..."
        cd "$BUILD_PATH/feeds/luci/applications" || return
...
_sync_luci_lib_docker() {
    local lib_path="$BUILD_PATH/feeds/luci/libs/luci-lib-docker"
    local repo_url="https://github.com/lisaac/luci-lib-docker.git"

    if [ ! -d "$lib_path" ]; then
        echo "正在同步 luci-lib-docker..."
        mkdir -p "$BUILD_PATH/feeds/luci/libs" || return
        cd "$BUILD_PATH/feeds/luci/libs" || return
...
update_dockerman() {
    local path="$BUILD_PATH/feeds/luci/applications/luci-app-dockerman"
    ...
    if [ -d "$path" ]; then
        ...
        if declare -F docker_stack_sync_dockerman_nftables_compat >/dev/null 2>&1; then
            docker_stack_sync_dockerman_nftables_compat "$BUILD_PATH" "0" || return 1
        fi
...
add_quickfile() {
    ...
    local target_dir="$BUILD_PATH/package/emortal/quickfile"
...
update_argon() {
    ...
    local dst_theme_path="$BUILD_PATH/feeds/luci/themes/luci-theme-argon"
    local tmp_dir
    tmp_dir=$(mktemp -d)
...
update_package() {
    # 根据上游版本信息刷新包版本与哈希。
    local dir=$(find "$BUILD_PATH/package" \( -type d -o -type l \) -name "$1")
    if [ -z "$dir" ]; then
        return 0
    fi
    local branch="$2"
