...
set_build_signature() {
    local file="$BUILD_PATH/feeds/luci/modules/luci-mod-status/htdocs/luci-static/resources/view/status/include/10_system.js"

    [ -f "$file" ] || return

    if ! grep -q "build by Kirayo" "$file"; then
        sed -i \
            "s#luciversion || ''#(luciversion || '') + ' / build by Kirayo'#g" \
            "$file"
    fi
}

update_menu_location() {
    local samba4_path="$BUILD_PATH/feeds/luci/applications/luci-app-samba4/root/usr/share/luci/menu.d/luci-app-samba4.json"
...
update_nginx_ubus_module() {
    local makefile_path="$BUILD_PATH/feeds/packages/net/nginx/Makefile"
