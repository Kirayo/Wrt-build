...
set_custom_task() {
    local sh_dir="$BUILD_PATH/package/base-files/files/etc/init.d"
...
install_opkg_distfeeds() {
    local emortal_def_dir="$BUILD_PATH/package/emortal/default-settings"
...
update_script_priority() {
    local qca_drv_path="$BUILD_PATH/package/feeds/nss_packages/qca-nss-drv/files/qca-nss-drv.init"
    if [ -d "${qca_drv_path%/*}" ] && [ -f "$qca_drv_path" ]; then
        sed -i 's/START=.*/START=88/g' "$qca_drv_path"
    fi

    local pbuf_path="$BUILD_PATH/package/kernel/mac80211/files/qca-nss-pbuf.init"
...
fix_openssl_ktls() {
    local config_in="$BUILD_PATH/package/libs/openssl/Config.in"
    if [ -f "$config_in" ]; then
...
fix_opkg_check() {
    ...
    local opkg_dir="$BUILD_PATH/package/system/opkg"
...
install_pbr_cmcc() {
    local pbr_pkg_dir="$BUILD_PATH/package/feeds/packages/pbr"
...
fix_quectel_cm() {
    local makefile_path="$BUILD_PATH/package/feeds/packages/quectel-cm/Makefile"
    local cmake_patch_path="$BUILD_PATH/package/feeds/packages/quectel-cm/patches/020-cmake.patch"
...
set_nginx_default_config() {
    local nginx_config_path="$BUILD_PATH/feeds/packages/net/nginx-util/files/nginx.config"
...
EOF
    ...
    local nginx_template="$BUILD_PATH/feeds/packages/net/nginx-util/files/uci.conf.template"
...
\tclient_body_temp_path /mnt/tmp;" "$nginx_template"
    ...
    local luci_support_script="$BUILD_PATH/feeds/packages/net/nginx/files-luci-support/60_nginx-luci-support"
...
update_uwsgi_limit_as() {
    local cgi_io_ini="$BUILD_PATH/feeds/packages/net/uwsgi/files-luci-support/luci-cgi_io.ini"
    local webui_ini="$BUILD_PATH/feeds/packages/net/uwsgi/files-luci-support/luci-webui.ini"
