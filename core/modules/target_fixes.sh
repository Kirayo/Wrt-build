...
fix_default_set() {
    ...
    if [ -d "$BUILD_PATH/feeds/luci/collections/" ]; then
        find "$BUILD_PATH/feeds/luci/collections/" -type f -name "Makefile" -exec sed -i "s/luci-theme-bootstrap/luci-theme-$THEME_SET/g" {} \;
    fi

    install -Dm544 "$BASE_PATH/patches/990_set_argon_primary" "$BUILD_PATH/package/base-files/files/etc/uci-defaults/990_set_argon_primary"
    install -Dm544 "$BASE_PATH/patches/991_custom_settings" "$BUILD_PATH/package/base-files/files/etc/uci-defaults/991_custom_settings"
    install -Dm544 "$BASE_PATH/patches/992_set-wifi-uci.sh" "$BUILD_PATH/package/base-files/files/etc/uci-defaults/992_set-wifi-uci.sh"

    if [ -f "$BUILD_PATH/package/emortal/autocore/files/tempinfo" ]; then
        if [ -f "$BASE_PATH/patches/tempinfo" ]; then
            \cp -f "$BASE_PATH/patches/tempinfo" "$BUILD_PATH/package/emortal/autocore/files/tempinfo"
        fi
    fi
}
...
fix_miniupnpd() {
    local miniupnpd_dir="$BUILD_PATH/feeds/packages/net/miniupnpd"
...
change_dnsmasq2full() {
    if ! grep -q "dnsmasq-full" $BUILD_PATH/include/target.mk; then
...
fix_mk_def_depends() {
    sed -i 's/libustream-mbedtls/libustream-openssl/g' $BUILD_PATH/include/target.mk 2>/dev/null
    if [ -f $BUILD_PATH/target/linux/qualcommax/Makefile ]; then
        sed -i 's/wpad-openssl/wpad-mesh-openssl/g' $BUILD_PATH/target/linux/qualcommax/Makefile
    fi
}


fix_kconfig_recursive_dependency() {
    local file="$BUILD_PATH/scripts/package-metadata.pl"
    if [ -f "$file" ]; then
...
remove_something_nss_kmod() {
    local ipq_mk_path="$BUILD_PATH/target/linux/qualcommax/Makefile"
    local target_mks=("$BUILD_PATH/target/linux/qualcommax/ipq60xx/target.mk" "$BUILD_PATH/target/linux/qualcommax/ipq807x/target.mk")
...
update_affinity_script() {
    local affinity_script_dir="$BUILD_PATH/target/linux/qualcommax"
...
apply_hash_fixes() {
    fix_hash_value \
        "$BUILD_PATH/package/feeds/packages/smartdns/Makefile" \
...
update_ath11k_fw() {
    local makefile="$BUILD_PATH/package/firmware/ath11k-firmware/Makefile"
    ...
    local ipq60_target="$BUILD_PATH/target/linux/qualcommax/ipq60xx/target.mk"
    local ipq807_target="$BUILD_PATH/target/linux/qualcommax/ipq807x/target.mk"
...
change_cpuusage() {
    local luci_rpc_path="$BUILD_PATH/feeds/luci/modules/luci-base/root/usr/share/rpcd/ucode/luci"
    local qualcommax_sbin_dir="$BUILD_PATH/target/linux/qualcommax/base-files/sbin"
    local filogic_sbin_dir="$BUILD_PATH/target/linux/mediatek/filogic/base-files/sbin"
    ...
    local old_script_path="$BUILD_PATH/package/base-files/files/sbin/cpuusage"
    if [ -f "$old_script_path" ]; then
        rm -f "$old_script_path"
    fi

    if [ -d "$BUILD_PATH/target/linux/qualcommax" ]; then
        install -Dm755 "$BASE_PATH/patches/cpuusage" "$qualcommax_sbin_dir/cpuusage"
    fi
    if [ -d "$BUILD_PATH/target/linux/mediatek" ]; then
...
update_nss_pbuf_performance() {
    local pbuf_path="$BUILD_PATH/package/kernel/mac80211/files/pbuf.uci"
...
update_nss_diag() {
    local file="$BUILD_PATH/package/kernel/mac80211/files/nss_diag.sh"
...
fix_compile_coremark() {
    local file="$BUILD_PATH/feeds/packages/utils/coremark/Makefile"
    if [ -d "$(dirname "$file")" ] && [ -f "$file" ]; then
...
update_dnsmasq_conf() {
    local file="$BUILD_PATH/package/network/services/dnsmasq/files/dhcp.conf"
    if [ -d "$(dirname "$file")" ] && [ -f "$file" ]; then
...
add_backup_info_to_sysupgrade() {
    local conf_path="$BUILD_PATH/package/base-files/files/etc/sysupgrade.conf"
...
fix_rust_compile_error() {
    if [ -f "$BUILD_PATH/feeds/packages/lang/rust/Makefile" ]; then
        sed -i 's/download-ci-llvm=true/download-ci-llvm=false/g' "$BUILD_PATH/feeds/packages/lang/rust/Makefile"
    fi
}
